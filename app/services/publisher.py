...
class PostPublisherService:
    ...
    async def publish(self, scheduled_post_id: uuid.UUID) -> bool:
        ...
        try:
            ...
            # Trigger first-publish celebration email (non-blocking)
            try:
                from app.workers.onboarding_tasks import send_first_publish_celebration
                from app.models.content import DraftContent
                from sqlalchemy import select, func
                count_q = await self._db.execute(
                    select(func.count(DraftContent.id)).where(
                        DraftContent.workspace_id == sp.workspace_id,
                ...
                ws_result = await self._db.execute(
                    select(Workspace).where(Workspace.id == sp.workspace_id)
                )
                ws = ws_result.scalar_one_or_none()
                if ws and ws.owner_id and total == 1:
                    send_first_publish_celebration.apply_async(
                        kwargs={"user_id": str(ws.owner_id)},
                        queue="beat",
                        countdown=5,
                    )
            except Exception:
