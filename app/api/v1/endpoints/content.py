...
from app.core.workspace_deps import get_workspace_or_403, WorkspaceAccess
...
async def regenerate_content(
    ...
    service = ApprovalService(db=db)
    # Module 9: credit gate
    _credit_svc = CreditService(db=db)
    await _credit_svc.assert_can_use(
        workspace_id=request.workspace_id if hasattr(body, "workspace_id") else request.workspace_id,
        user_id=current_user,
        action_type=CreditActionType.CAPTION_GENERATE,
    )
    try:
...
async def plan_calendar(
    ...
    # Module 9: credit gate
    _credit_svc = CreditService(db=db)
    await _credit_svc.assert_can_bulk_generate(user_id=current_user)
    await _credit_svc.assert_can_use(
        workspace_id=request.workspace_id,
        user_id=current_user,
        action_type=CreditActionType.CALENDAR_PLAN,
    )
    try:
